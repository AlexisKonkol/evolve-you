  ...
  useEffect(() => {
    const generate = async () => {
      try {
        ...
        const compassResults = sessionStorage.getItem("compassResults") || '';
        
        // Always try to get Spark Profile data
        let sparkData = "";
        ...
        if (user) {
          ...
          if (spark) {
            sparkData = `
SPARK PROFILE:
- Loses track of time: ${spark.q1 || "Not provided"}
- Would do for free: ${spark.q2 || "Not provided"}
- Most proud of: ${spark.q3 || "Not provided"}
- Impact they want: ${spark.q4 || "Not provided"}
- Best environment: ${spark.q5 || "Not provided"}
- Life that feels right: ${spark.q6 || "Not provided"}`;
          }
        }
        ...
        let userContext = "";
        if (compassAnswers) {
          userContext += `COMPASS ANSWERS:\n${compassAnswers}\n\n`;
        }
        if (compassResults) {
          userContext += `COMPASS RESULTS:\n${compassResults}\n\n`;
        }
        if (sparkData) {
          userContext += sparkData;
        }

        // If no data at all, provide a generic prompt
        if (!userContext) {
          userContext = "No specific profile data available. Generate a general Edge Profile.";
        }
        ...
        const response = await fetch("https://api.anthropic.com/v1/messages", {
          ...
          body: JSON.stringify({
            ...
            max_tokens: 1500,
            system: `You are NAVO. Based on the user's profile data, identify their navigation style and generate their Edge Profile.
...
Return ONLY the JSON. No markdown. No backticks. No explanation.`,
            messages: [{
              role: "user",
              content: `Generate an Edge Profile based on this user data:\n\n${userContext}`
            }]
          })
        });
        ...
        console.error('EdgeProfile error:', err);
  ...
  if (error || !profile) return (
    <div style={{ minHeight:"100vh", background:"#0B0F1A", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:16, fontFamily:"'Plus Jakarta Sans',sans-serif", padding:32 }}>
      ...
      <p style={{ color:"white", fontSize:18, margin:0, textAlign:"center" }}>
        {error ? `Error: ${error}` : "Something went wrong generating your edge profile."}
      </p>
      {error && (
      ...
      <button onClick={() => window.location.reload()} style={{ background:"#6366F1", color:"white", border:"none", borderRadius:4, padding:"12px 28px", fontFamily:"'Plus Jakarta Sans',sans-serif", fontSize:11, fontWeight:600, letterSpacing:"0.14em", textTransform:"uppercase", cursor:"pointer" }}>
        Try Again
      </button>
    </div>
  );
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#0B0F1A", fontFamily:"'Plus Jakarta Sans',sans-serif", position:"relative" }}>
      ...
      {/* Glow */}
      <div style={{ position:"fixed", top:0, left:"50%", transform:"translateX(-50%)", width:"100%", height:400, background:"radial-gradient(ellipse 700px 350px at 50% -60px, rgba(99, 102, 241,0.13) 0%, transparent 70%)", pointerEvents:"none", zIndex:0 }} />
      ...
      <div style={{ maxWidth:640, margin:"0 auto", padding:"52px 32px 80px", position:"relative", zIndex:1, opacity: visible ? 1 : 0, transition:"opacity 0.4s ease" }}>
        ...
        <div style={{ textAlign:"center", marginBottom:40 }}>
          ...
          <h1 style={{ fontFamily:"'Playfair Display',serif", fontSize:"clamp(32px,5vw,44px)", fontWeight:400, color:"white", letterSpacing:"-0.02em", lineHeight:1.1, margin:"0 0 12px" }}>
            You are <span style={{ color:"#6366F1", fontStyle:"italic" }}>{profile.navigationStyle}.</span>
          </h1>
        ...
        {/* NAVO Sees */}
        <div style={{ background:"rgba(99, 102, 241,0.06)", border:"1px solid rgba(99, 102, 241,0.2)", borderRadius:16, padding:24, marginBottom:16, animation:"fadeUp 0.5s ease 0.1s both" }}>
          <div style={{ fontSize:10, fontWeight:600, letterSpacing:"0.2em", textTransform:"uppercase", color:"#6366F1", marginBottom:12 }}>
        ...
        <div style={{ background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.07)", borderRadius:16, padding:24, marginBottom:16, animation:"fadeUp 0.5s ease 0.3s both" }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            {profile.directions.map((d, i) => (
              <div key={i} style={{ padding:"14px 16px", background:"rgba(255,255,255,0.03)", borderRadius:12, borderLeft:`3px solid ${i === 0 ? "#6366F1" : "rgba(99, 102, 241,0.3)"}` }}>
        ...
        <div style={{ background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.07)", borderRadius:16, padding:24, marginBottom:28, animation:"fadeUp 0.5s ease 0.4s both" }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            {profile.recommendedModules.map((mod, i) => (
              <div key={i} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"14px 16px", background: i === 0 ? "rgba(99, 102, 241,0.06)" : "rgba(255,255,255,0.03)", border: i === 0 ? "1px solid rgba(99, 102, 241,0.2)" : "1px solid rgba(255,255,255,0.07)", borderRadius:12 }}>
                <div>
        ...
        <div style={{ display:"flex", gap:12, animation:"fadeUp 0.5s ease 0.5s both" }}>
          <button onClick={() => navigate("/compass")} style={{ flex:1, background:"#6366F1", color:"white", border:"none", borderRadius:4, padding:"15px", fontFamily:"'Plus Jakarta Sans',sans-serif", fontSize:11, fontWeight:600, letterSpacing:"0.16em", textTransform:"uppercase", cursor:"pointer", transition:"all 0.2s" }}
            onMouseEnter={e => { e.currentTarget.style.opacity="0.9"; e.currentTarget.style.transform="translateY(-1px)"; }}
            onMouseLeave={e => { e.currentTarget.style.opacity="1"; e.currentTarget.style.transform="translateY(0)"; }}>
            Start {profile.recommendedModules[0]} →
          </button>
          <button onClick={handleSave} style={{ background:"transparent", color: saved ? "#6366F1" : "rgba(255,255,255,0.35)", border: saved ? "1px solid rgba(99, 102, 241,0.4)" : "1px solid rgba(255,255,255,0.12)", borderRadius:4, padding:"15px 24px", fontFamily:"'Plus Jakarta Sans',sans-serif", fontSize:11, fontWeight:500, letterSpacing:"0.1em", textTransform:"uppercase", cursor:"pointer", transition:"all 0.2s", whiteSpace:"nowrap" }}>
