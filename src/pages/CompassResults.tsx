...
Generate their Edge Profile now.`
  ...
  if (error) return (
    <div style={{ minHeight:"100vh", background:"#0A0A0A", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:16, fontFamily:"'Plus Jakarta Sans',sans-serif", padding:32 }}>
      ...
      <button onClick={() => navigate("/compass")} style={{ marginTop:8, background:"#FF6B2B", color:"white", border:"none", borderRadius:4, padding:"12px 28px", fontFamily:"'Plus Jakarta Sans',sans-serif", fontSize:12, fontWeight:600, letterSpacing:"0.14em", textTransform:"uppercase", cursor:"pointer" }}>
        Try Again
      </button>
    </div>
  );
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#0A0A0A", fontFamily:"'Plus Jakarta Sans',sans-serif", position:"relative" }}>
      ...
      {/* Orange glow */}
      <div style={{ position:"fixed", top:0, left:"50%", transform:"translateX(-50%)", width:"100%", height:400, background:"radial-gradient(ellipse 700px 350px at 50% -60px, rgba(255,107,43,0.14) 0%, transparent 70%)", pointerEvents:"none", zIndex:0 }} />
      ...
      <div style={{ maxWidth:640, margin:"0 auto", padding:"52px 32px 80px", position:"relative", zIndex:1 }}>
        ...
        {cards.map((card, i) => (
          <div key={i} style={{
            background: card.reframe ? "rgba(255,107,43,0.08)" : "rgba(255,255,255,0.04)",
            border: card.reframe ? "1px solid rgba(255,107,43,0.2)" : "1px solid rgba(255,255,255,0.07)",
            borderLeft: card.hero ? "3px solid #FF6B2B" : card.reframe ? "1px solid rgba(255,107,43,0.2)" : "1px solid rgba(255,255,255,0.07)",
            borderRadius:16, padding:28, marginBottom:16,
            animation:`fadeUp 0.5s ease ${i * 0.12}s both`,
          }}>
            <div style={{ marginBottom:14 }}>
              <span style={{ fontSize:10, fontWeight:600, letterSpacing:"0.2em", textTransform:"uppercase", color: card.hero || card.reframe ? "#FF6B2B" : "rgba(255,255,255,0.45)" }}>
                {card.label}
              </span>
            </div>
            <p style={{
